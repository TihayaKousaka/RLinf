...
```text
ready_for_online = update_step >= warmup_post_collect_updates
```
...
注意：`warmup_post_collect_updates` 控制 actor 何时上线；`actor_loss_warmup_updates` 控制 loss 权重。两者单位都是 critic update，但作用不同。
...
desired_total_updates =
  warmup_post_collect_updates + transition_cycles * update_epoch
...
rollout:
  expert_model:
    model_path: ".../global_step_8000/actor"
    act_as_vla_reference: True
```

`act_as_vla_reference: True` 表示 expert 只作为强 VLA reference，不加载 Stage2 actor。
...
| `train/replay_buffer/size` | 当前 replay buffer 大小 | 判断是否超过 warmup_min_size |
...
| `warmup_post_collect_updates` | actor 上线前足够训练 |
