...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_rlt_maniskill",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=10, discrete_state_input=True
        ),
        data=LeRobotRLTManiSkillDataConfig(
...
def _override_with_model_path(config: TrainConfig, model_path: str) -> TrainConfig:
    ...
    if (
        ...
        assets_dir = getattr(data_config.assets, "assets_dir", None)
        # Preserve explicit absolute asset locations, e.g. dataset-side norm stats.
        # Relative assets dirs still follow the model checkpoint as before.
        override_assets_dir = assets_dir is None or not os.path.isabs(assets_dir)
        data_config = dataclasses.replace(
            data_config,
            assets=(
                dataclasses.replace(data_config.assets, assets_dir=model_path)
                if override_assets_dir
                else data_config.assets
            ),
        )
